...
def resized_center_crop(img, scale=0.875, size=(1024, 1024), interpolation=Image.BILINEAR):
...
class DistortImageFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        save_path = '/share/data/lang/users/dan/ImageNet-C-1024/' + self.method.__name__ + \
...
def glass_blur(x, severity=1):
    ...
    for i in range(c[2]):
        for h in range(1024 - c[1], c[1], -1):
            for w in range(1024 - c[1], c[1], -1):
...
def defocus_blur(x, severity=1):
    ...
    channels = np.array(channels).transpose((1, 2, 0))  # 3x1024x1024 -> 1024x1024x3
...
def fog(x, severity=1):
    ...
    x = np.array(x) / 255.
    max_val = x.max()
    x += c[0] * plasma_fractal(wibbledecay=c[1])[:1024, :1024][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    frost = cv2.resize(cv2.imread(filename),(1500,1500))
    # randomly crop and convert to rgb
    x_start, y_start = np.random.randint(0, frost.shape[0] - 1024), np.random.randint(0, frost.shape[1] - 1024)
    frost = frost[x_start:x_start + 1024, y_start:y_start + 1024][..., [2, 1, 0]]
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(x, cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(1024, 1024, 1) * 1.5 + 0.5)
...
def pixelate(x, severity=1):
    ...
    x = x.resize((int(1024 * c), int(1024 * c)), PILImage.BOX)
    x = x.resize((1024, 1024), PILImage.BOX)
...
class CoffeeLeavesDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_name = os.path.join(self.image_list[idx])
        # Open image
        ...
        if not os.path.exists(self.method.__name__):
            os.makedirs(self.method.__name__)
        if not os.path.exists(self.method.__name__+ '/' + str(self.severity)):
            os.makedirs(self.method.__name__+ '/' + str(self.severity))
        if not os.path.exists(self.method.__name__+ '/' + str(self.severity)+ '/' + str(label)):
            os.makedirs(self.method.__name__+ '/' + str(self.severity)+ '/' + str(label))

        Image.fromarray(np.uint8(image)).resize((2048,1024)).save(save_path, quality=100, optimize=True)
        
        # Multitask
        return (np.uint8(image), label)
...
val_transforms=trn.Compose([trn.Resize((1024, 1024))])
csv_file = "dataset/dataset.csv"
images_dir = "../../coffee-datasets/leaf/images"
fold = 1
select_clf = 2
